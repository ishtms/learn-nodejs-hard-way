...
class Logger {
    ...
    async init() {
        ...
        /**Expcted filename looks like this -> "LogTar_2023-08-26T14:46:11.log" this works well with Linux
        * but it thorws an error if it is Windows because windows doesn't allow the colon ":" in filename
        * So we could write this line an replace the dots and clolons with a -
        * const file_name = const file_name=this.#config.file_prefix+ new Date().toISOString().replace(/[\..+::+]/,"-")+".log";
        * Expcted filename looks like this -> "LogTar_2023-08-26T14-46-11.log" 
        */
