...
> The code for the entire section can be found [at the code/chapter_04.2 directory](/code/chapter_04.2)
...
We've been using `require` throughout our codes, which just imports a module, and exposes all it's exported functionality - whether it's a variable, a function or a class. You may think of it as a function since `require('node:path')` indeed looks like a function call.
...
Perfect! Everything seems to be working now. The logs are being logged, and we can be proud of it! But wait, the logs aren't useful. We don't know what are those logs, when did we write them and what function called the `logger.error()` method?
