...
This is called a **stack trace**. The `throw new Error()` prints the whole stack trace unwinding back through the series of function calls that were made leading up to the point where the error occurred. Each function call is recorded in reverse order, starting from the function that directly caused the error and going back to the initial entry point of the script. 

This trace of function calls, along with their corresponding file paths and line numbers, provides developers with a clear trail to follow, aiding in the identification of where and how the error originated. 
...
How could we use the information that we learnt above to get the line number from the client's code? Can you think about it?
...
However this time, there are 2 more lines. If you start reading from the bottom to top, that's what actually happened in a step wise step manner. The useful part is not actually the line at the bottom. It is at the 4th line (I am counting the `Error` at the top of the stack trace as a line too).
...
Looking at the code that we just wrote in our `Logger.#log` method, it looks really awful. Hard to understand what we actually want, and why are we even throwing an Error? Someone who's not aware of our code might think that it's a redundant method, get rid of it.

We should make it even better. First, we are going to create a helper method which is going to get the essential info from our stack trace.
...
In this line, we're getting the part of the string after the `at `. We don't care about the `at `. At the last line, we're returning the meta data that we need to show on the logs.
...
This all seem to work pretty good. We now have helpful logs. Before we start using this logging library in our personal projects, there are a lot of things that need to be taken care of. Especially logging crashes, `SIGINT`, `SIGTERM` as well as proper utilization of the `file_handle`.
